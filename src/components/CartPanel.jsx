/**
 * The cart itself: every line with quantity controls, the live total and the
 * "Place Order" action.
 *
...
/**
 * @param {{ onPlaceOrder: () => void, onClose?: () => void }} props - Checkout
 *   handler, plus an optional close handler shown only in the mobile sheet.
 * @returns {JSX.Element} The cart panel.
 */
export default function CartPanel({ onPlaceOrder, onClose }) {
  const { cartItems, clearCart } = useCafeteria();
    ...
    <section className="surface-card flex max-h-[calc(100vh-8rem)] flex-col overflow-hidden lg:max-h-[calc(100vh-9rem)]">
      <header className="flex items-center justify-between gap-3 border-b border-clay/60 px-5 py-4">
        ...
        {onClose ? (
          ...
          !isCartEmpty && (
            <button type="button" onClick={clearCart} className="button-quiet">
              Clear
            </button>
          )
        )}
        ...
        <>
          <ul className="flex-1 divide-y divide-clay/50 overflow-y-auto px-5">
            {cartItems.map((cartItem) => (
              <CartLineItem key={cartItem.id} cartItem={cartItem} />
            ))}
          </ul>
          ...
          <footer className="border-t border-clay/60 bg-cream/60 px-5 py-4">
            <div className="mb-4 flex items-baseline justify-between">
              ...
              <span className="text-2xl font-semibold text-cocoa">{formatRupees(cartTotal)}</span>
            </div>

            <button type="button" onClick={onPlaceOrder} className="button-primary w-full">
              Place Order
            </button>
